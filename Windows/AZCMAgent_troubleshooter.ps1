...
class ArcLogger {
    ...
    [void]writeLine([ArcLoggerLevel]$ArcLoggerLevel, [string]$source,[string]$message) {
        try {
            $this.writeLineToLog($ArcLoggerLevel,$source,$message)
            $this.writeLineToConsole($ArcLoggerLevel,$message,$true)
        } catch {
    ...
    [void]writeLineToLog([ArcLoggerLevel]$ArcLoggerLevel, [string]$source,[string]$message) {
    ...
    [void]WriteTabletoLog([ArcLoggerLevel]$ArcLoggerLevel, [string]$source,[PSCustomObject]$table, [ScriptBlock] $expression) {
        ...
        try {
            foreach ($row in $table) {
                ...
                $this.writeLineToLog($ArcLoggerLevel,$source,$parsed_row)
            }
        } catch {
    ...
    [void]writeLineToConsole([ArcLoggerLevel]$ArcLoggerLevel,[string]$message,[bool]$UseNewLine) {
...
function Show-AgentDetails {
    $Global:Logger.WriteLine([ArcLoggerLevel]::Info,"Show-AgentDetails","Azure Connected Machine Agent Details")
    $Global:Logger.WriteLine([ArcLoggerLevel]::Info,"Show-AgentDetails","$('-'*37)")
    $Global:Logger.WriteLine([ArcLoggerLevel]::Info,"Show-AgentDetails","Machine Name:`t`t$($Global:AgentDetails.resourceName)")
    $Global:Logger.WriteLine([ArcLoggerLevel]::Info,"Show-AgentDetails","Version:`t`t`t$($Global:AgentDetails.agentVersion)")
    $Global:Logger.WriteLine([ArcLoggerLevel]::Info,"Show-AgentDetails","Region:`t`t`t`t$($Global:AgentDetails.location)")
    
    # Check if Proxy is enabled. If so - show proxy information
    if (![System.String]::IsNullOrEmpty($Global:AgentDetails.httpsProxy)) {
        $Global:Logger.WriteLine([ArcLoggerLevel]::Info,"Show-AgentDetails","HTTP Proxy:`t`t`t`t$($Global:AgentDetails.httpsProxy)")

        # Check if Proxy Bass is enabled. If so - show bypass tags
        if (![System.String]::IsNullOrEmpty($Global:AgentDetails.proxyBypass)) {
        $Global:Logger.WriteLine([ArcLoggerLevel]::Info,"Show-AgentDetails","Proxy Bypass:`t`t`t`t$($Global:AgentDetails.proxyBypass)")
        }
        else {
            $Global:Logger.WriteLineToLog([ArcLoggerLevel]::Info,"Show-AgentDetails","Proxy Bypass:`t`t`t`tnot configured")
        }
    }
    else {
        $Global:Logger.WriteLineToLog([ArcLoggerLevel]::Info,"Show-AgentDetails","HTTP Proxy:`t`t`t`tnot configured")
    }
    

    $Global:Logger.WriteLine([ArcLoggerLevel]::Info,"Show-AgentDetails","Status:`t`t`t`t$($Global:AgentDetails.status)")
    ...
    if (![System.String]::IsNullOrEmpty($Global:AgentDetails.agentErrorCode)) {
        $Global:Logger.WriteLine([ArcLoggerLevel]::Warn,"Show-AgentDetails","Agent Error Code:`t$($Global:AgentDetails.agentErrorCode): $($Global:AgentDetails.agentErrorDetails)")
    }
    ...
    $Global:Logger.WriteLineToConsole([ArcLoggerLevel]::Info,"Services:",$true)
...
function Collect-WinEventLogs {
    ...
    $Global:Logger.writeLineToLog([ArcLoggerLevel]::Debug,"Collect-WinEventLogs","Collecting event log: `'$Source`'")
    ...
    try 
    {
        ...
        $Global:Logger.writeLineToLog([ArcLoggerLevel]::Debug,"Collect-WinEventLogs","Found event log: `'$log_file`'")

        $output_file = $event_logs_folder_path + "\$Source.evtx"
        Copy-Item -Path $log_file -Destination $output_file
        $Global:Logger.writeLineToLog([ArcLoggerLevel]::Debug,"Collect-WinEventLogs","$Source event log was saved to: `"$output_file`"")
    }
    catch
    {
        $Global:Logger.writeLine([ArcLoggerLevel]::Error,"Collect-WinEventLogs","Failed to collect event log: $_")
    }
}

function collect-NetworkCheck {
    $Global:Logger.writeLineToLog([ArcLoggerLevel]::Debug,"collect-NetworkCheck","Performing network check")
    ...
    try {
        ...
        $Global:Logger.writeLineToLog([ArcLoggerLevel]::Debug,"collect-NetworkCheck","Resolving DNS for `"gbl.his.arc.azure.com`" to determine if there's a use of Private Link")
        ...
        if ($his_ip,'(10|172).\d+.\d+.\d+') {
            $Global:Logger.writeLineToLog([ArcLoggerLevel]::Debug,"collect-NetworkCheck","Private Link Identified, will use --use-private-link flag")

            $Global:Logger.writeLineToLog([ArcLoggerLevel]::Debug,"collect-NetworkCheck","Running command: azcmagent check --location $($Global:AgentDetails.location) --use-private-link")
            $check_result = azcmagent check -j --location $Global:AgentDetails.location --use-private-link | convertfrom-json
        }
        else {
            $Global:Logger.writeLineToLog([ArcLoggerLevel]::Debug,"collect-NetworkCheck","Private Link not found.")
            $Global:Logger.writeLineToLog([ArcLoggerLevel]::Debug,"collect-NetworkCheck","Running command: azcmagent check --location $($Global:AgentDetails.location)")
            $check_result = azcmagent check -j --location $($Global:AgentDetails.location) | convertfrom-json
        }

        # Display results to console
        $Global:Logger.writeLineToConsole([ArcLoggerLevel]::Info,"Network Availability check",$true)
        $Global:Logger.writeLineToConsole([ArcLoggerLevel]::Info,"--------------------------",$false)
        $check_result | select endpoint,reachable,Required,Private,tls,'proxy status' | ft

        # Write table results to the log
        $Global:Logger.WriteTabletoLog([ArcLoggerLevel]::Info,"collect-NetworkCheck", $check_result, {param($x) return "Endpoint: `"$($x.endpoint)`" | Reachable: `"$($x.reachable)`" | Private IP: `"$($x.private)`" | TLS: `"$($x.tls)`" | Proxy Status: `"$($x.'proxy status')`""})
    }
    catch {
        $Global:Logger.writeLine([ArcLoggerLevel]::Error,"Collect-NetworkCheck","Failed perform network check: $_")
    }
}

function Zip-Collection {
    $zip_name = "$Global:WorkDir.zip"
    $Global:Logger.writeLineToLog([ArcLoggerLevel]::Debug,"Zip-Collection","Compressing all logs")
    ...
    try {
        ...
        $Global:Logger.writeLineToConsole([ArcLoggerLevel]::Info,"Logs saved into `"$zip_name`"",$true)

        # Open the directory where the zip file was saved.
        & "explorer" "/select,$zip_name"
    }
    catch {
        $Global:Logger.writeLineToConsole([ArcLoggerLevel]::Error,"Failed to zip work directory: $_",$true)
    }
...
function Collect-Logs {
    ...
    $Global:Logger.writeLineToLog([ArcLoggerLevel]::Debug,"Collect-Logs","Running: azcmagent logs -o `"$zip_name`"")
    
    
    try {
        azcmagent logs -o "$zip_name" | Out-Null
        
        $Global:Logger.writeLineToLog([ArcLoggerLevel]::Debug,"Collect-Logs","Logs were saved to: `"$zip_name`"")
    }
    catch {
        $Global:Logger.writeLine([ArcLoggerLevel]::Error,"Collect-Logs","Failed to collect agent logs: $_")
    }
}

function Collect-OSInfo {
    $Global:Logger.writeLineToLog([ArcLoggerLevel]::Debug,"Collect-OSInfo","Collecting OS information")
    ...
    try {
        ...
        $Global:Logger.writeLineToLog([ArcLoggerLevel]::Debug,"Collect-OSInfo","Saved OS information to file: `"$report_file`"")
    }
    catch {
        $Global:Logger.writeLineToLog([ArcLoggerLevel]::Debug,"Collect-OSInfo","Failed to collect OS info: $_")
    }
}

function Collect-Extesions {
    $extension_report_folder = "C:\ProgramData\GuestConfig\extension_reports\"
    $Global:Logger.writeLineToLog([ArcLoggerLevel]::Debug,"Collect-Extesions","Collecting Extensions reports from folder: `"$extension_report_folder`"")
    ...
    try {
        Get-ChildItem $extension_report_folder | ForEach-Object {
            ...
            $Global:Logger.writeLineToLog([ArcLoggerLevel]::Info,"Collect-Extesions","Name: `"$($ext.Name)`"`t| Provisioning State: `"$($ext.ProvisioningState)`"`t| Status Level: `"$($ext.StatusLevel)`"`t| Status Message: `"$($ext.StatusMessage)`"")
        }
    } catch {
        $Global:Logger.writeLine([ArcLoggerLevel]::Error,"Collect-Extesions","Failed to extensions reports: $_")
    }
}
