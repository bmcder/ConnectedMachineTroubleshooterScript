...
function Collect-OSInfo {
    ...
    try {
        ...
        # Add Root CAs
        Add-Content -Path $report_file -Value "`n`nTrusted Root Certification Authorities`n======================================"

        (gci cert:\localmachine\AuthRoot).Issuer |% {
            Add-Content -Path $report_file -Value (($_ -split ',')[0] -split '=')[-1]
        }
